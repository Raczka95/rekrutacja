...
def lista_ogloszen():
    ...
    for ogloszenie in ogloszenia:
        if ogloszenie.data_wygasniecia:
            days_remaining = (ogloszenie.data_wygasniecia - datetime.utcnow().date()).days
            ogloszenie.status = f"{days_remaining} dni do końca" if days_remaining >= 0 else "Wygasło"
